...
def join_text_fragments(texts: Iterable[str]) -> str:
    """
    Join text fragments into a single string to match terms against.

    Texts are joined by newlines so that terms are not matched across their
    boundaries, and duplicates are dropped to keep the string short.
    """
